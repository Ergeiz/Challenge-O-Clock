...
<details>
<summary> Énoncé de l'atelier </summary>
...
Je vais choisir un réseau publique (`203.0.113.0/24`) pour le switch où les gens se connecteront au VPN   
<br/>
## Plan Paris  
Lan : `10.0.0.0/22` (1022 adresse, de `10.0.0.1` à `10.0.3.254`, ça laisse de la marge)  
DMZ et R&D : `192.168.0.0` (252 adresses, même réseau dans des batiments séparés relié en fibre)  
...
Le materiel a été réparti de cette façon:  
...
Je configure tous les switch, par exemple avec celui du réseau R&D
```
switch> enable
switch# conf t
...
Ensuite pour les routages sur le router de Paris, je détermine une route statique ciblant le réseau 203.0.112.0 vers le router VPN et une route par défaut vers le router de Lille :

```
RouterParis> enable
RouterParis# conf t
RouterParis (config)# ip route 203.0.113.0 255.255.255.0 92.56.78.2
RouterParis (config)# ip route 0.0.0.0 0.0.0.0 92.12.34.2
```
...
Les paquets qui voudront sortir tout en ne ciblant pas le réseau VPN iront vers le router de Lille  
Je détermine une route par défaut sur le router de Lille en direction du router de Paris, de même avec le router vpn en direction du router de Paris
...
```
Je fais de même pour les 3 sous-réseau, normalement il ne reste plus rien dans la config

Maintenant dans le serveur DHCP je vais faire un pool pour chaque sous-réseau
